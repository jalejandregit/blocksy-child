...
@ini_set('error_log', '/home/u710927107/domains/triapedres.com/public_html/wp-content/elm-error-logs/php-errors.log');
...
function link_click_counter() {
   ...
   if ( isset( $_POST['nonce'] ) &&  isset( $_POST['post_id'] ) && wp_verify_nonce( $_POST['nonce'], 'link_click_counter_'.$_POST['post_id'] ) ) {
	...
	switch ( $nonce ) {
		case 1:
			$resultado =  'Nonce is less than 12 hours old';
			break;
		case 2:
			$resultado =  'Nonce is between 12 and 24 hours old';
			break;
		default:
			$resultado =  'Nonce is invalid' ;
	}
	*/
    //exit();
    echo $resultado;
...
function link_click_head(){
	...
	jQuery(function ($) {
	   $('a.count').on('click', function (e) {
			...
			var post='';
			var id = $(this).attr('id');
			post =id.replace('countable_link_', '');
			var ajax_options = {
							action: 'procesa_link',
            				post_id: post,
							nonce: '$nonce'
...
/* es modifica wp_enqueue_style canviant versio per date
 * https://triapedres.com/wp-content/themes/blocksy-child/style.css?ver=1.9.11 
--> get_template_directory()siempre recupera el archivo del tema principal.
--> get_stylesheet_directory()requiere que el archivo exista en el tema secundario*/
...
/*echo get_stylesheet_directory_uri() . '/style.css?ver=' . $cache_buster;*/
}
...
function my_theme_enqueue_styles() {
    $parenthandle = 'blocksy-child'; 
    $theme = wp_get_theme();
    wp_enqueue_style( $parenthandle, get_stylesheet_uri() , 
...
function desa_post_Excerpt(){
	...
	if($post_tipo==='post'){
		...
		$rosavents = orientacio($post_orient);
		...
		/*
		$post_excerpt = '<strong>'.$post_link_title.'</strong><br/>'.$taula_dades.'<br/><a class="count" id="countable_link_'.$post_id.'-2"  href="'.$href.'" target="_blank"><img border="0" height="210" src="'.$thumb.'" width="154"></a>';
		*/
			...
			if ( ( $post_id ) AND ( $post_excerpt ) ) {
				...
				/***** save tag orientacio ***/
				//wp_set_post_tags( $post_id, $post_orient, true );
				wp_set_post_tags( $post_id, $post_orient );	
				remove_action('save_post', 'desa_post_Excerpt', 50); // Unhook this function so it doesn't loop infinitely

				wp_update_post( $post_array );

				add_action( 'save_post', 'desa_post_Excerpt', 50); // Re-hook this function

			}
			}
}

//add_action('save_post', 'desa_post_Excerpt', 50);
...
function desa_post_type_Excerpt() {
	...
	if($post_tipo==='post'){
		...
		if (defined('WP_DEBUG') && WP_DEBUG) {
            error_log("desa_post_type_Excerpt: " . print_r($post_id, true));
        }
...
*/
	...
	if ( ( $post_id ) AND ( $post_excerpt ) ) {
		...
		if (defined('WP_DEBUG') && WP_DEBUG) {
            error_log("2--> desa_post_Custom_Excerpt: " . print_r($post_excerpt, true));
        }
		update_post_meta($post_id, 'location', $name_location);
		/***** save tag orientacio ***/
		//wp_set_post_tags( $post_id, $post_orient, true );
		...
		//add_action( 'save_post', 'desa_post_Custom_Excerpt', 50); // Re-hook this function
...
function getTableDades($autor, $autorurl ,$orientacio_html,$location_html){
	/*
	$table1 = '<table style="width:100%" class="table_dades"><tbody><tr><td style="width:5%"><i class="fas fa-user-pen"></i></td><td>'.$autor.'</td><td style="width:15%" rowspan="2">'.$orientacio_html.'</td></tr><tr>';
*/	
...
/***** Address(lat + long) **/
function getAddress_proves($lat, $lon) {
	return 'AAAAAAAA';
}

function getAddress_sospita($latitude,$longitude){
...
function migas_de_panProves(){
	//ok echo 'Categoria Seleccionada:'.get_query_var('cat').'<br>';
	//ok echo 'Categoria name:'.get_query_var('category_name').'<br>';
	
	/***** OK tag_cloud_by_category ***/
	//tag_cloud_by_category(get_query_var('cat'));
	//
	/***** OK show_posts_from_process_cat_tree ***/
	//show_posts_from_process_cat_tree(get_query_var('cat'));
	//
	//**** OK getCrumbsCategoryAncestors() ***/
	//getCrumbsCategoryAncestors();
	
	//--- RG getCategoriasHirerycal No filtra por categoria */
	//getCategoriasHirerycal(get_query_var('cat'));

	
	//echo 'x'. count(get_the_category());
	//$tags = get_tags_in_use($category_ID, 'id');
	//$tags = sortCategories(get_the_category());
	$tags = get_the_category();
	
	//$tags = get_categories_hierarchical(get_the_category());
	//$tags = wp_list_sort( get_the_category( $post_id ), 'term_id' );
	//$tags = wp_list_sort( get_the_category(get_query_var('cat')), 'term_id' );
	...
	//$i=0;
	 foreach ($tags as $char):
		//$result .='-'.$char->term_id;
		$result .=', '.$char->name; 
		$result_id .="-".$char->term_id;
		//echo "$char->term_id is " . is_int($char->term_id) . "<br>";
		//$result =$char->term_id;
		/*
		$value = $char->term_id;
		if($i==0){
		...
		*/
		//$i++;
	endforeach;
	//echo tag_cloud_by_category($result);
...
/* lista desordenada
	...
	echo $result_id;
*/	
		//echo wp_list_categories('child_of=0&depth=0&title_li=&orderby=id&exclude=""&hide_empty=1&current_category=24');
...
//function getCrumbsTags($category_ID,$current){
...
function get_queried_object2test() {
	/*$tag = get_query_var('tag') ;
	echo $tag.get_queried_object_id();
	echo get_stylesheet_uri();*/
	
	/*$tag_selected = $tag = get_query_var('tag');
	echo $tag_selected;*/
	
	$tags = get_tags_in_use($category_ID, 'id');
	$array_orden = array(3,7,2,4,0,6,1,5);
	$tags_ordered = orderArrayTags($tags, $array_orden);
	//print_r($tags_ordered);
	//echo $tags;
	//var_dump($tags_ordered  );
